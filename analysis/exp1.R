...
condition_meta <- tribble(
  ~cue,~short,
  "all_cues", "AC",
  "no_eyes", "NE",
  "no_face", "NF",
  "no_hands", "NH",
  "no_hands_no_mouth", "NHM",
  "no_mouth", "NM",
  "only_body", "NHF",
  "only_face", "NHB",
  "only_hands", "NFB"
  ...
  mutate(
    short = factor(short, levels = c("AC", "NE", "NM", "NF", "NFB", "NH", "NHM", "NHF", "NHB"))
  )
...
metrics %>%
  ...
  select(Phenomenon = phenomenon, N = n, `Primary Cue`, `Secondary Cue`, AC:NHB) %>%
...
ac_correctness = correctness %>%
  filter(inference_condition == "AC") %>%
  rename(ac_correct = correct) %>%
  select(-inference_condition, -nll_diff, -nll_diff_scaled)

correctness %>% 
  inner_join(ac_correctness) %>%
  filter(inference_condition != "AC") %>%
  group_by(phenomenon, only_nmm, inference_condition) %>%
  # summarize(n=n())
  nest() %>%
...
sig_x <- tibble(
  phenomenon = c("Numbers", "Fingerspelling", "Classifiers", "Wh-Questions", "Positive vs. Negation", "Conditionals"),
  x = c(0.055, 0.10, 0.09, 0.18, 0.04, 0.07)
) %>%
  mutate(
    phenomenon = factor(
      phenomenon,
...
metrics %>%
  ...
  geom_text(data = sigs %>% inner_join(sig_x), aes(x = x, y = inference_condition, label = sig), 
            color = "red", family = "Helvetica", size = 5, fontface="bold") +
  facet_wrap(~phenomenon, scales = "free_x", nrow=2) +
  facetted_pos_scales(
    x = list(
      phenomenon == "Classifiers" ~ scale_x_continuous(limits = c(-0.025,0.1), breaks=c(-0.025,0,0.025, 0.05, 0.075, 0.1), labels = c("-0.025", "0", "0.025", "0.05", "0.075", "0.1")),
      phenomenon == "Conditionals" ~ scale_x_continuous(limits = c(0, 0.08)),
      # str_detect(phenomenon, "Conditionals<br>") ~ scale_x_continuous(limits = c(0, 0.05))
...
ggsave("images/asllrp-surprisal-difference-inference.pdf", height = 6.74, width = 15.28, dpi=300, device=cairo_pdf)
ggsave("images/asllrp-surprisal-difference-inference-cue.pdf", height = 6.74, width = 15.28, dpi=300, device=cairo_pdf)
ggsave("images/asllrp-surprisal-difference-inference.svg", height = 6.74, width = 15.28, dpi=300)
